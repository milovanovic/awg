...
class SingleAWGFFTChainTester(   
  dut: SingleAWGFFTChain[FixedPoint],
  ...
  val f1 = Figure("Single AWG FFT Chain Output")
  ...
  f1.saveas(s"test_run_dir/single_awg_fft_chain.pdf")
}


class SingleAWGFFTChainSpec extends FlatSpec with Matchers {
  ...
  it should "Test Single AWG FFT Chain" in {
    val lazyDut = LazyModule(new SingleAWGFFTChain(paramsPLFG, paramsNCO, paramsFFT, AddressSet(0x001000, 0xFF), AddressSet(0x000000, 0x0FFF), AddressSet(0x001100, 0xFF), AddressSet(0x001200, 0xFF), AddressSet(0x001300, 0xFF), beatBytes) {
    })
    chisel3.iotesters.Driver.execute(Array("-tiwv", "-tbn", "verilator", "-tivsuv"), () => lazyDut.module) {
      c => new SingleAWGFFTChainTester(lazyDut, AddressSet(0x001000, 0xFF), AddressSet(0x000000, 0x0FFF), AddressSet(0x001100, 0xFF), AddressSet(0x001200, 0xFF), AddressSet(0x001300, 0xFF), beatBytes)
    } should be (true)
  }
}
